...
public class JeuPage extends Page{
	...
	public JeuPage(ClientController _controller){
		...
		try{
			if(getController().getJoueur().getLogin().equals(getController().getJeuEnCours().getAdmin().getLogin())){
				...
				if(getController().getJeuEnCours().getStatut().equals(Statut.OUVERT) || getController().getJeuEnCours().getStatut().equals(Statut.PLEIN)){
					middlePane.add(bLancer, new GBC(5,1,2,1).setFill(GridBagConstraints.NONE).setInsets(10,5,10,1));
				}
			}
		}catch(RemoteException rE){
			System.out.println(rE);
	...
	private void lancer(){
		...
		try{
			...
			getController().getJeuEnCours().setStatut(Statut.ENCOURS);
			bLancer.hide();
			bLancer.updateUI();
		}catch(RemoteException rE){
			System.out.println(rE);
    ...
    private void refresh(){
		...
		try{
	        if(getController().getJeuEnCours().getStatut().equals(Statut.ENCOURS)){
				for(Joueur joueur : getController().getListeJoueursJeu()){
